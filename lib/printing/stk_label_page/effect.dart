...
void _onPrinterStk(Action action, Context<StkLabelState> ctx) {
  ...
  info['loc_to_show'] = (info['loc'] ?? '').toString();
  info['lot_to_show'] = (ctx.state.lotNumberController.text ?? '').toString();
  info['part_no_to_show'] = (info['part_no'] ?? '').toString();
  info.addAll({
    'lot': initials.lot + (ctx.state.lotNumberController.text ?? '').toString()
  });
